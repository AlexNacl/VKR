    ...
    void InitializeClick()
    {
        ...
        if (locationSpawn.GetBusy() == false && UnitId < 10)
        {
            ...
            if (UnitId == 0)
            {
                ...
                if (PlayerManager.GetMil() == true)
                {
                    if (PlayerManager.MercCount() >= 5 || PlayerManager.MercHadCount() >= 12)
                    {
                    UIManager.UpdatePlayerText("Mercinary limit exceeded");
                    return;
                    }
                }
                else if (PlayerManager.MercCount() >= 4 || PlayerManager.MercHadCount() >= 10)
                {
                    UIManager.UpdatePlayerText("Mercinary limit exceeded");
                    return;
                }
            ...
            if (UnitId == 2)
            {
                ...
                if (PlayerManager.GetLaw() == true)
                {
                    if (PlayerManager.SpyCount() >= 4)
                    {
                    UIManager.UpdatePlayerText("Spy limit exceeded");
                    return;
                    }
                }
                else if (PlayerManager.SpyCount() >= 3)
                {
                    UIManager.UpdatePlayerText("Spy limit exceeded");
                    return;
                }
                temp = 4+PlayerManager.SpyCount()*2;
                if (Playpoints < temp)
                {
                    UIManager.UpdatePlayerText("Not enough money");
                    return;
                }
                PlayerManager.PlayPointsConsumed(temp);
        ...
        else if (UnitId >= 10 && inBuildingMenu)
        {
            ...
            if (UnitId == 10)
            {
                ...
                PlayerManager.PlayPointsConsumed(6);
            }
            if (UnitId == 11)
            {
                if (Playpoints < 4)
