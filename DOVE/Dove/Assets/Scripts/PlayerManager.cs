...
public class PlayerManager : NetworkBehaviour
{
    ...
    public  override void OnStartClient()
    {
        ...
        ExternalInfo = GameObject.Find("ExternalInfo").GetComponent<Text>();
    ...
    public void RpcWhosTurn()
    {
        ...
        if (!pm.isMyTurn)
        {
            UIManager.UpdateTurnText("Enemy Turn");
    ...
    void countingTerr(bool first)
    {
        ...
        for (int i = 0; i <= LocationSockets.Count-1; i++)
        {
            ...
            Debug.Log("its "+regCount+" and "+ LocationSockets.Count);
        ...
        foreach (GameObject unit in UnitsAr)
        {
            if (unit != null)
            {
                ...
                if (unitScript != null)
                {
                    ...
                    {
                        if (unitScript.isOwnerFirst != parentRegion.IsOwnerFirst || parentRegion.IsOwnerNone)
                        {
                            unitScript.MinusHP(2);
                        }
                    }
                }
